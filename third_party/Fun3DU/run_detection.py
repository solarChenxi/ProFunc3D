...
from utils.hf_mirror import setup_hf_endpoint
...
def make_mask_index(args: DictConfig):
    ...
    print(
        f"Processing {len(visit_ids)} visits (split {args.dataset.split}): {visit_ids}"
    )
...
def run_owl2_rsam(args: argparse.Namespace, bs: int = 2):
    """
    ...
    # Hydra override: detection_bs=1|2|...
    if hasattr(args, "detection_bs") and args.detection_bs is not None:
        bs = int(args.detection_bs)

    owl_m, owl_p, sam_m, sam_p = init_detection(device)
    print(f"Detection batch size: {bs}")

    visits2videos = io.get_visit_to_videos(root, split)
    visits = sort_alphanumeric(list(visits2videos.keys()))
    visits = select_visits(visits, args.dataset)

    print(
        f"Running detection for {len(visits)} visits (split {args.dataset.split}): {visits}"
    )
    parser = DataParser(root, split)
    ...
    for visit_id in tqdm(visits):
        ...
        # optional: only one video (hydra: video_id=47333319)
        if getattr(args, "video_id", None) is not None:
            want = str(args.video_id)
            video_list = [v for v in video_list if str(v) == want]
            if not video_list:
                print(f"Skip visit {visit_id}: video_id={want} not found")
                continue
        ...
        for video_id in video_list:
            ...
            # optional frame index shard (hydra: frame_start=0 frame_end=360)
            fs = getattr(args, "frame_start", None)
            fe = getattr(args, "frame_end", None)
            if fs is not None or fe is not None:
                fs = 0 if fs is None else int(fs)
                fe = len(frame_ids) if fe is None else int(fe)
                frame_ids = frame_ids[fs:fe]
                rgb_paths = rgb_paths[fs:fe]
                print(
                    f"Frame shard {visit_id}/{video_id}: indices [{fs},{fe}) -> {len(frame_ids)} frames"
                )
            ...
            for i_b in range(NB):
                ...
                # skip frames already saved (resume after OOM)
                pending_paths, pending_frames = [], []
                for path, fid in zip(paths_i, frames_i):
                    out_path = join(
                        args.dataset.root,
                        args.dataset.split,
                        visit_id,
                        video_id,
                        args.mask_type,
                        f"{video_id}_{fid}.npz",
                    )
                    if not os.path.exists(out_path):
                        pending_paths.append(path)
                        pending_frames.append(fid)
                if not pending_paths:
                    continue

                imgs = [Image.open(path) for path in pending_paths]

                all_imgs_data = process_detection(
                    owl_m, owl_p, sam_m, sam_p, imgs, prompts
                )
                # free SAM activations between batches on V100
                if device == "cuda":
                    torch.cuda.empty_cache()

                # iterate over data about images
                for frame_id, frame_data in zip(pending_frames, all_imgs_data):
                    mask_desc_ids = list()
...
def main(args: DictConfig):
    ...
    # RobustSAM-large + hires RGB: bs=10 OOMs on 32GB V100; default 1 is safe.
    run_owl2_rsam(args, bs=1)
