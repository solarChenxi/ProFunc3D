...
def molmo_pipeline(args: DictConfig):
    ...
    parser = DataParser(args.dataset.root, args.dataset.split)
    # get_visits() returns one row per video (duplicates); keep unique visit order
    visits = sort_alphanumeric(list(dict.fromkeys(parser.get_visits())))
    visit_ids = select_visits(visits, args.dataset)

    print(
        f"Running molmo on {len(visit_ids)} visits (split {args.dataset.split}): {visit_ids}"
    )
    ...
    for visit_id in visit_ids:
        ...
        # optional within-visit desc sharding (for finishing large visits across GPUs)
        desc_indices = _parse_visit_indices(getattr(args, "desc_indices", None))
        if desc_indices is not None:
            pairs = [
                (desc_data[i], llm_data[i])
                for i in desc_indices
                if 0 <= i < len(desc_data)
            ]
            print(
                f"  {visit_id}: desc shard {len(pairs)}/{len(desc_data)} indices={desc_indices}"
            )
        else:
            d0 = getattr(args, "desc_start", None)
            d1 = getattr(args, "desc_end", None)
            if d0 is not None or d1 is not None:
                d0 = 0 if d0 is None else int(d0)
                d1 = len(desc_data) if d1 is None else int(d1)
                pairs = list(zip(desc_data[d0:d1], llm_data[d0:d1]))
                print(f"  {visit_id}: desc range [{d0},{d1}) -> {len(pairs)}/{len(desc_data)}")
            else:
                pairs = list(zip(desc_data, llm_data))

        for desc_annot, llm_annot in pairs:
            desc_id = desc_annot["desc_id"]
