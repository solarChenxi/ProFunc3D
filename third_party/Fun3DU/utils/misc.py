...
def np_normalize(array: np.ndarray) -> np.ndarray:
    ...
    m_array = array.copy()
    # constant / empty heatmap → avoid NaN (would crash Open3D viz)
    if amax == amin:
        return np.zeros_like(m_array, dtype=np.float64)
...
def _parse_visit_indices(raw) -> Optional[List[int]]:
    """Parse dataset.visit_indices from hydra (list / comma-string / None)."""
    if raw is None:
        return None
    # OmegaConf may give ListConfig / empty string
...
def select_visits(visits: Sequence[str], dataset_cfg) -> List[str]:
    """Select visit subset via visit_indices (preferred) or start/end range.

    visit_indices allows non-contiguous shards for load-balanced multi-GPU runs
    (e.g. isolate a heavy visit like 467115 instead of packing it with neighbors).
    """
    indices = _parse_visit_indices(getattr(dataset_cfg, "visit_indices", None))
    if indices is not None:
        out = []
        for i in indices:
            if i < 0 or i >= len(visits):
                raise IndexError(
                    f"visit_indices entry {i} out of range for {len(visits)} visits"
                )
            out.append(visits[i])
        return out

    start = 0 if getattr(dataset_cfg, "start", None) is None else int(dataset_cfg.start)
    end = len(visits) if getattr(dataset_cfg, "end", None) is None else int(dataset_cfg.end)
    return list(visits[start:end])
